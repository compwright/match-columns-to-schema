...
function matchColumnsToSchema ({ schema, columns, rows }) {
    ...
    columns.mergeMap((column) => columnValues(rows, column.column, 100)
        .toArray()
        .map((values) => ({ values, ...column }))
    ).subscribe(columnsWithValues)

    // Match each column against the schema
    const schemaFields = Object.entries(schema).map(([field, { aliases, validator }]) => {
        return (validator ? columnsWithValues : columns)
            .map(({ name, column, values }) => ({
                field,
                header: name,
                index: column,
                similarityScore: max(stricmp_array(name, [field].concat(aliases))),
                validationScore: validator ? validationScore(values, validator) : Infinity
            }))
...
function columnValues(rows, index, limit = Infinity) {
    const observer = rows.map(({ values }) => values[index])
        .filter((str) => !!str)
...
const stricmp_array = (a, bs = []) => bs.filter(b => b)
    .map(b => stringSimilarity.compareTwoStrings(a.toLowerCase(), b.toLowerCase()))
